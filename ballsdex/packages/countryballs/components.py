...
class CountryballNamePrompt(Modal, title=f"Catch this {settings.collectible_name}!"):
    ...
    async def on_error(self, interaction: discord.Interaction, error: Exception, /) -> None:
        log.exception("An error occured in kirbo catching prompt", exc_info=error)
        if interaction.response.is_done():
            await interaction.followup.send(
                f"An error occured with this {settings.collectible_name}."
                "\nPlease do not catch it as it may break the bot further."
            )
        else:
    ...
    async def on_submit(self, interaction: discord.Interaction["BallsDexBot"]):
        ...
        if self.ball.catched:
            await interaction.response.send_message(
                f"Sorry {interaction.user.mention} , I was caught already...", ephemeral=True)        
        ...
        if self.name.value.lower().strip() in possible_names:
            ...
            await interaction.followup.send(
                ...
                f"`(ID: #{ball.pk:0X}, ATK: {ball.attack_bonus:+}%, HP: {ball.health_bonus:+}%)`\n\n"
                f"{special}"
            )
            self.button.disabled = True
            await interaction.followup.edit_message(self.ball.message.id, view=self.button.view)
        else:
            await interaction.response.send_message(f"Sorry {interaction.user.mention}, wrong name! Try again.\n\n"
            f"**You entered:** `{self.name.value.lower().strip()}`", ephemeral=True)
...
class CatchButton(Button):
    def __init__(self, ball: "CountryBall"):
        super().__init__(style=discord.ButtonStyle.primary, label="Catch kirbo!")
        self.ball = ball

    async def callback(self, interaction: discord.Interaction):
        if self.ball.catched:
            await interaction.response.send_message(f"Sorry {interaction.user.mention} , I was caught already...", ephemeral=True)
        else:
