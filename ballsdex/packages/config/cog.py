...
activation_embed = discord.Embed(
    ...
    f"read and accept the [Terms of Service and Privacy Policy]({settings.privacy_policy}).\n\n"
    ...
    f"- Do not attempt to abuse the bot's internals\n"
    f"**Not respecting these rules will lead to a blacklist**\n\n"
    f"__Please join the official KirboDex server for bot updates and other important stuff!__",
)
...
class Config(commands.GroupCog):
    ...
    async def channel(
        ...
        """
        Set or change the channel where kirbos will spawn.
        """
    ...
    async def disable(self, interaction: discord.Interaction):
        """
        Disable or enable kirbos spawning.
        """
        guild = cast(discord.Guild, interaction.guild)  # guild-only command
        user = cast(discord.Member, interaction.user)
        if not user.guild_permissions.manage_guild:
