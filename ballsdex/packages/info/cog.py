...
class Info(commands.Cog):
    ...
    async def about(self, interaction: discord.Interaction):
        ...
        embed.description = (
            ...
            "This bot was inspired by **El Laggron**'s Ballsdex, consider supporting him on his "
            "[Patreon](https://patreon.com/retke) :heart:\n\n"
            f"[KirboDex Discord server]({settings.discord_invite}) • [Invite me]({invite_link}) • "
            f"[Source code and issues]({settings.github_link})\n"
            f"[Terms and Privacy Policy]({settings.privacy_policy})"
        )
    ...
    async def commands(self, interaction: discord.Interaction):
        ...
        assert self.bot.user
        embed = discord.Embed(
            title=f"{settings.bot_name} Discord Bot - Commands List", color=discord.Colour.blurple()
        )
        embed.set_thumbnail(url=self.bot.user.display_avatar.url)
