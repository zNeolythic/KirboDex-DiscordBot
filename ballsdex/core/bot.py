...
class BallsDexBot(commands.AutoShardedBot):
        ...
        if interaction.user.id in self.blacklist:
            if interaction.type != discord.InteractionType.autocomplete:
                await interaction.response.send_message(
                    ...
                    "\nYou can appeal this blacklist in #support in our support server: {}".format(
    ...
    async def on_command_error(
        ...
        if isinstance(exception, commands.CommandInvokeError):
            ...
            await context.send(
                "An error occured when running the command."
                "\nCreate a ticket in #support in the KirboDex Support Server if this persists. https://discord.gg/vUzHXxnuRs"
            )
            return
